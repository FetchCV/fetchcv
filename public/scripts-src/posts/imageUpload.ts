...
const previewArea = document.getElementById("upload-preview-area") as HTMLElement;
const alertDiv = document.querySelector(".alert") as HTMLElement;
const MAX_FILES = 10;
...
function displayPreview(file: File, parentElement: HTMLElement): void {
...
function dropHandler(event: DragEvent): void {
   ...
   if (filesProcessed > 0) {
      ...
      updateFileState();
      if (currentFiles.length === 0) updateFileState();
   }
}
